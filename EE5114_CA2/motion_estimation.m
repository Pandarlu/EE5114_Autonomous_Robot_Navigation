...
for i = 1:size(output_temp,1)-1
	...
	if (assoc_count >= 2)
        ...
        % Compute the centroids of the corresponding points P and Q
        P_bar = mean(P, 1); 
        Q_bar = mean(Q, 1); 
        ...
        [U, S, V] = svd(P_centered' * Q_centered );
        d = sign(det(V * U'));
        ...
        % Final orientation and position changes:
        theta = -atan2(R(2), R(1));
        translation = T(1:2);
