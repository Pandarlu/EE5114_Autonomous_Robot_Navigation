...
video_rec = 0; % Turn on/off video recording function 
...
for i = 1:size(output_temp,1)
	...
	for j = 1:particles_count
        ...
        % Retrieve motion estimates for the current iteration 'i'
        dx = motion_estimate(i).x; 
        dy = motion_estimate(i).y; 
        dtheta = motion_estimate(i).theta;

        % Apply rotation to all particles with (additive and proportional) noises 
        noisy_dtheta = dtheta + randn(1) * theta_noise_proportion * dtheta + randn(1) * theta_noise_add;

        % Apply translation to all particles with (additive and proportional) noises 
        noisy_dx = dx + randn(1) * translation_noise_proportion * dx + randn(1) * translation_noise_add;
        noisy_dy = dy + randn(1) * translation_noise_proportion * dy + randn(1) * translation_noise_add;

        particles(j).theta = slam_in_pi(theta + noisy_dtheta);
        particles(j).x = x + noisy_dx * cos(theta) - noisy_dy * sin(theta);
        particles(j).y = y + noisy_dx * sin(theta) + noisy_dy * cos(theta);
