...
% Split and merge algorithm for line extraction
    ...
    for i = 1:last
         %Calculate the distance of each point to the estimated line
         N = (x2 - x1) * (y1 - points(i).y) - (x1 - points(i).x) * (y2 - y1);
         D = sqrt((x2 - x1)^2 + (y2 - y1)^2);
         point_distances(i) = abs(N/D);

         %Check if this point is farther than the current winner
         if point_distances(i) > winner_value
             winner_value = point_distances(i);
             winner_index = i;
         end
    end
