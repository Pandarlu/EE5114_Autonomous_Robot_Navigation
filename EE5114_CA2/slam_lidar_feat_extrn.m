	...
	for i = 1:size(lines, 2)-1
		if (lines(i).p2.x == lines(i+1).p1.x && lines(i).p2.y == lines(i+1).p1.y)
			...
			if (lines(i).length > length_min && lines(i+1).length > length_min)
                ...
                va = [lines(i+1).p2.x - lines(i).p2.x, 
                      lines(i+1).p2.y - lines(i).p2.y];
                vb = [lines(i).p1.x - lines(i+1).p1.x,
                      lines(i).p1.y - lines(i+1).p1.y];
            
                cos_angle = dot(va, vb) / (norm(va) * norm(vb));
                angle = slam_in_pi(acos(cos_angle));
				...
				if (angle > deg2rad(60) && angle < deg2rad(120))
                    ...
                    va_unit = va / norm(va);  
                    vb_unit = vb / norm(vb);  
                    heading_vec = va_unit + vb_unit;
                    heading = slam_in_pi(atan2(heading_vec(2), heading_vec(1)));
